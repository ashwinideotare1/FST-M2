...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "peterp") // Pass request body from file
        ...
        response.then().body("username", equalTo("peterp"));
        response.then().body("firstName", equalTo("peter"));
        response.then().body("lastName", equalTo("parker"));
        response.then().body("email", equalTo("peterparker@mail.com"));
        response.then().body("password", equalTo("password123"));
        response.then().body("phone", equalTo("8806684747"));
    }
    ...
    public void deleteUser() throws IOException {
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "peterp") // Add path parameter
            .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("peterp"));
    }
}
